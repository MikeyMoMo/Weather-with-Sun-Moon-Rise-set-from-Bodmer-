...
void syncTime(void)
{
  ...
  if (nextSendTime < millis()) {
    if (year() > 2022)
      Serial.printf("It is %02i/%02i/%02i %02i:%02i:%02i and that is time to get an NTP update.\r\n",
                    month(), day(), year(), hour(), minute(), second());
    else
      Serial.println("Getting initial NTP time update.");
    // Get a random server from the pool
    WiFi.hostByName(ntpServerName, timeServerIP);
    nextSendTime = millis() + 43200000;  // Half a day is good.
